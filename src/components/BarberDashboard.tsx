...
export const BarberDashboard: React.FC<BarberDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "agenda" | "galeria" | "servicos" | "config"
  >("agenda");

  // Agenda State
  const [appointments, setAppointments] = useState<Appointment[]>([]);
  const [dateMode, setDateMode] = useState<
    "today" | "tomorrow" | "next7" | "future" | "specific" | "range" | "all"
  >("today");
  ...
  const [selectedAppDetail, setSelectedAppDetail] =
  ...
  const [editingService, setEditingService] =
    useState<Partial<ServiceItem> | null>(null);
  ...
  const [mediaSourceMode, setMediaSourceMode] = useState<"upload" | "url">(
    "upload",
  );
  ...
  // Firestore pushes a new snapshot whenever any device creates, edits or
  // cancels an appointment. Reapply the dashboard filters on every update.
  useEffect(() => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      const data = await localStore.getAppointments(filters);
  ...
  const fetchServices = () => {
    try {
      const data = localStore.getServices();
  ...
  const fetchGallery = () => {
    try {
      const data = localStore.getGallery();
      setGalleryList(data);
    } catch (e: any) {
      console.error(e);
    }
  ...
  const handleUpdateStatus = async (
    ...
    try {
      await localStore.updateAppointmentStatus(id, status);
      fetchAppointments();
  ...
  const handleDeleteAppointment = async (id: string) => {
    ...
    try {
      await localStore.deleteAppointment(id);
      fetchAppointments();
  ...
  const handleCreateManualAppointment = async (e: React.FormEvent) => {
    ...
    try {
      await localStore.createAppointment({
  ...
  const handleSaveService = (e: React.FormEvent) => {
    ...
    try {
      if (editingService.id) {
        localStore.updateService(editingService.id, editingService);
      } else {
        localStore.createService(editingService);
      }
  ...
  const handleSetFeatured = (id: string) => {
    try {
      const res = localStore.setFeaturedVideo(id);
  ...
  const handleSaveConfig = (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const updated = localStore.updateShopConfig(shopForm);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 pb-16 w-full max-w-full overflow-x-hidden">
      {/* Dashboard Top Header */}
      <div className="bg-zinc-900 border-b border-amber-500/20 py-6 px-4 sm:px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="hidden sm:block text-3xl font-black font-serif uppercase text-zinc-100">
              {config.name}
            </h1>
            <p className="text-xs text-zinc-400 mt-1">
              Gerencie seus horários, clientes, tabela de serviços e fotos da
              galeria.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all min-h-[44px]"
            >
          ...
          <div className="space-y-6">
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-3 border-b border-zinc-800/80">
                ...
                <div className="space-y-1.5 shrink-0">
                  ...
                  <div className="flex items-center gap-1 bg-zinc-950 p-1 rounded-xl border border-zinc-800 text-xs overflow-x-auto scrollbar-none">
                    {["todos", "pending", "completed", "cancelled"].map(
                      (st) => (
                        <button
                          key={st}
            ...
            {loadingApps ? (
              ...
              <div className="py-20 text-center bg-zinc-900 rounded-2xl border border-zinc-800 text-zinc-400 text-xs space-y-2">
                ...
                <p>
                  Você pode mudar o filtro acima ou adicionar um agendamento
                  manual.
                </p>
              </div>
            ) : (
          ...
          <div className="space-y-8">
                    ...
                    {newMediaUrl && mediaSourceMode === "upload" ? (
                        ...
                        <div className="flex items-center gap-3 overflow-hidden">
                          ...
                          <div className="min-w-0">
                            <p className="text-xs font-bold text-zinc-200 truncate">
                              {uploadFileName || "Arquivo selecionado"}
                            </p>
                      ...
                      <div className="relative border-2 border-dashed border-zinc-800 hover:border-amber-500/60 rounded-2xl p-6 text-center cursor-pointer transition-colors bg-zinc-950/60 group">
                        ...
                        <p className="text-xs font-bold text-zinc-200">
                          {isReadingFile
                            ? "Carregando arquivo..."
                            : "Clique ou arraste uma foto ou vídeo da galeria do seu dispositivo"}
                        </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {galleryList.map((item) => (
                <div
                  key={item.id}
                  className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden p-4 space-y-3"
                >
        ...
        {activeTab === "servicos" && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold font-serif uppercase text-amber-400">
                  Tabela de Serviços
                </h3>
                <p className="text-xs text-zinc-400">
                  Edite preços, durações e adicione novos serviços.
                </p>
              </div>
        ...
        {activeTab === "config" && (
          <div className="max-w-3xl bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-6">
            <div>
              ...
              <p className="text-xs text-zinc-400">
                Altere o nome da barbearia, endereço, telefone e horário de
                funcionamento.
              </p>
            </div>
            ...
            <form onSubmit={handleSaveConfig} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setShopForm({ ...shopForm, slogan: e.target.value })
                  }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setShopForm({ ...shopForm, instagram: e.target.value })
                    }
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-zinc-100 focus:border-amber-500 focus:outline-none"
                  />
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setShopForm({ ...shopForm, openingHours: e.target.value })
                  }
                  className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-zinc-100 focus:border-amber-500 focus:outline-none"
                />
              </div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditingService({
                        ...
                        price:
                          e.target.value === "consult"
                            ? null
                            : editingService.price || 50,
                      })
                    }
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-2.5 text-zinc-100 focus:border-amber-500"
                  >
